    ...
    const float* bottom_rois, float* top_data)
{
    CUDA_1D_KERNEL_LOOP(index, nthreads)
    {
        ...
        {
            printf("n=%d, channels=%d, c=%d, num_rois=%d, index=%d, roi_batch_ind=%d, roi_b=%d, roi_start_d=%d, roi_end_d=%d\n", 
                    n, channels, c, num_rois, index, roi_b, roi_batch_ind, roi_start_d, roi_end_d);
            return;
        }
