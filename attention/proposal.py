...
def proposal_layer(score_output, feature_mask, gts=None, test_mode=False, ss_prob=0., 
    ...
    for k in range(batch_size):
        ...
        if len(scores) > 1:
            ...
            props = [(x, y, 1, scores[x:y+1].mean()*math.sqrt(pstarts[x]*pends[y])) for x in starts for y in ends if x < y and scores[x:y+1].mean() > 0.3]
            if scores.mean() > 0.3:
                props += [(0, len(scores)-1, 1, scores.mean()*math.sqrt(pstarts[0]*pends[-1]))]
            # import pdb; pdb.set_trace()
        else:
            props = [(0, len(scores)-1, 1, scores.mean()*math.sqrt(pstarts[0]*pends[-1]))]
        bboxes.extend(props)
