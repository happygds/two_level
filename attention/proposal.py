...
def proposal_layer(score_output, feature_mask, gts=None, test_mode=False, ss_prob=0.,
    ...
    labels = np.zeros((batch_size, rpn_post_nms_top, 4))
    ...
    if test_mode:
        ...
        for k in range(batch_size):
            ...
            # else:
            _, bboxes_dict[k], rois_iou_dict[k] = gen_prop(
    ...
    # rois_relative_pos[:, :, :, 0] = 10. * (rois_start[:, np.newaxis, :] - rois_start[:, :, np.newaxis]) / rois_dura[:, np.newaxis, :].clip(1e-14)
    # rois_relative_pos[:, :, :, 1] = 10. * (rois_end[:, np.newaxis, :] - rois_end[:, :, np.newaxis]) / rois_dura[:, np.newaxis, :].clip(1e-14)
    rois_relative_pos[:, :, :, 0] = np.log((np.abs(
        rois_start[:, np.newaxis, :] - rois_start[:, :, np.newaxis]) / rois_dura[:, np.newaxis, :].clip(1e-3)).clip(1e-3))
    rois_relative_pos[:, :, :, 1] = np.sign(
        rois_start[:, np.newaxis, :] - rois_start[:, :, np.newaxis])
    rois_relative_pos[:, :, :, 2] = np.log((np.abs(
        rois_end[:, np.newaxis, :] - rois_end[:, :, np.newaxis]) / rois_dura[:, np.newaxis, :].clip(1e-3)).clip(1e-3))
    rois_relative_pos[:, :, :, 3] = np.sign(
        rois_end[:, np.newaxis, :] - rois_end[:, :, np.newaxis])
