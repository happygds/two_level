...
def proposal_layer(score_output, feature_mask, gts=None, test_mode=False, ss_prob=0., 
    ...
    rois_relative_pos = np.zeros((batch_size, rpn_post_nms_top, rpn_post_nms_top, 3)).astype('float32')
    ...
    rois_relative_pos[:, :, :, 1] = np.log * (rois_dura[:, :, np.newaxis] / rois_dura[:, np.newaxis, :]).clip(1e-3))
    rois_relative_pos[:, :, :, 2zeal] = np.sign(rois_cent[:, np.newaxis, :] - rois_cent[:, :, np.newaxis])
