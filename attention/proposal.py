...
def proposal_layer(score_output, feature_mask, gts=None, test_mode=False, ss_prob=0., 
    ...
    rois_relative_pos[:, :, :, 0] = 50. * ((rois_start[:, np.newaxis, :] - rois_start[:, :, np.newaxis]) / rois_dura[:, np.newaxis, :].clip(1e-14)).clip(-1., 1.)
    rois_relative_pos[:, :, :, 1] = 50. * ((rois_end[:, np.newaxis, :] - rois_end[:, :, np.newaxis]) / rois_dura[:, np.newaxis, :].clip(1e-14)).clip(-1., 1.)
