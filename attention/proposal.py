...
def proposal_layer(score_output, feature_mask, gts=None, test_mode=False, ss_prob=0., 
    ...
    xx1, yy1 = np.maximum(start_rois[:, np.newaxis, :1] , start_rois[:, :1, np.newaxis]), np.minimum(end_rois[:, np.newaxis, 1:2], end_rois[:, 1:2, np.newaxis])
    ...
    rois_relative_pos[:, :, :, 1] = 20. * (rois_end[:, np.newaxis, :] - rois_end[:, :, np.newaxis]) / rois_dura[:, np.newaxis, :].clip(1e-14) * (rois_iou > 0.)
