...
def proposal_layer(score_output, feature_mask, gts=None, test_mode=False, ss_prob=0., 
    ...
    for k in range(batch_size):
        ...
        if len(scores) > 1:
            ...
            props = [(x, y, 1, max(scores[x:y].mean() - \
            0.5*(scores[max(0, int(round(6*x/5.-y/5.))):max(int(round(4*x/5.+y/5.)), int(round(6*x/5.-y/5.))+1)].mean() + \
            scores[max(0, int(round(4*y/5.+x/5.))):max(int(round(6*y/5.-x/5.)), int(round(4*y/5.+x/5.))+1)].mean()), 0)*(pstarts[x]*pends[y])) \
            for x in starts for y in ends if x+1 < y and scores[x:y].mean() > 0.3]
