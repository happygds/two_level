...
class Rank_Criterion(nn.Module):
    ...
    def forward(self, x, y, mask):
        ...
        bs = x.size()[0]
        x = torch.cat([x, torch.zeros((bs, 1)), torch.ones((bs, 1))], dim=1)
        y = torch.cat([y, torch.zeros((bs, 1)), torch.ones((bs, 1))], dim=1)
        ...
        pred = (F.relu((y.unsqueeze(1) - y.unsqueeze(2)) - (x.unsqueeze(1) - x.unsqueeze(2)) )) * mask * y_mask
