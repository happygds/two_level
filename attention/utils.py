...
class Rank_Criterion(nn.Module):
    ...
    def forward(self, x, y, mask):
        ...
        pred = F.relu(self.epsilon - (x.unsqueeze(1) - x.unsqueeze(2))) * y_mask * mask.unsqueeze(1) * mask.unsqueeze(2)
        output = (pred ** 2).sum(2).sum(1) / mask.sum(1).clamp(eps)
...
def pos_embedding(position_mat, feat_dim, wave_length=1000.):
    feat_range = torch.arange(0, feat_dim / 6)
    dim_mat = torch.pow(wave_length, (6. / feat_dim) * feat_range)
