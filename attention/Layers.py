...
class Local_EncoderLayer(nn.Module):
    ...
    def forward(self, enc_input, input_mask=None):
        ...
        local_output = self.local_pos_ffn(local_output).view(shp[0], shp[1] // self.num_local, self.num_local, shp[2]).transpose(1, 2).view(-1, shp[1] // self.num_local, shp[2])

        enc_output, enc_slf_attn = self.slf_attn(
            local_output, local_output, local_output, attn_mask=slf_attn_mask)
        enc_output = self.pos_ffn(enc_output).view(shp[0], self.num_local, shp[1] // self.num_local, shp[2]).transpose(1, 2).view(shp)
        return enc_output, enc_slf_attn
