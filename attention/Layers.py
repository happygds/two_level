...
class ROI_Relation(nn.Module):
    ...
    def forward(self, features, start_rois, end_rois, rois, rois_mask, rois_pos_emb):
        ...
        start_ratio = start_ratio.unsqueeze(2).float() * torch.arange(self.start_pool_size).view((1, 1, -1)).float().cuda().requires_grad_(False)
        ...
        end_ratio = end_ratio.unsqueeze(2).float() * torch.arange(self.start_pool_size).view((1, 1, -1)).float().cuda().requires_grad_(False)
