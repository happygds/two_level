...
def gen_prop(v):
    ...
    for i in range(1, N):
        ...
        actness = (actness * roi_scores + args.wg * sel_actness * sel_roi_scores) / (1. + args.wg) * (actness > 0.)
        # roi_scores = (roi_scores + args.wg * sel_roi_scores) / (1. + args.wg) * (actness > 0.)
        rois = (rois + args.wg * sel_rois) / (1. + args.wg) * (actness > 0.).reshape((-1, 1))
    # actness, roi_scores = actness ** (1./N), roi_scores ** (1./N)

    bboxes = [(roi[0] / float(frm_cnt) * v.duration, roi[1] / float(frm_cnt) * v.duration,
               1, act_score, roi_score)
...
print('AR@50 is {:.6f}, AR@100 is {:.6f}, AUC is {:.6f}'.format(
    ar_at_prop[49], ar_at_prop[99], auc))
