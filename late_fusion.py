...
weights_list1 = list(np.arange(0.1, 1.01, 0.1)) + list(np.arange(1.01, 5.01, 1.))
weights_list2 = list(np.arange(0.1, 1.01, 0.1)) + list(np.arange(1.01, 5.01, 1.))
    ...
    for merge_weight2 in weights_list2:
        ...
        def gen_prop(v):
            ...
            if N == 2:
                ...
                actness = (actness + merge_weight1 * sel_actness) / (1. + merge_weight1) * (actness > 0.)
                roi_scores = (roi_scores + merge_weight1 * sel_roi_scores) / (1. + merge_weight1) * (actness > 0.)
                rois = (rois + merge_weight1 * sel_rois) / (1. + merge_weight1) * (actness > 0.).reshape((-1, 1))
            elif N == 3:
                i = 1
                this_rois, this_actness, this_roi_scores, _ = score_list[i][vid]
                this_ious = iou(rois, this_rois)
                ...
                sel_rois, sel_actness, sel_roi_scores = this_rois[argmax_ious],\
                    this_actness[argmax_ious], this_roi_scores[argmax_ious]
                actness = (actness + merge_weight2 * sel_actness) / (1. + merge_weight2) * (actness > 0.)
                roi_scores = (roi_scores + merge_weight2 * sel_roi_scores) / (1. + merge_weight2) * (actness > 0.)
                rois = (rois + merge_weight2 * sel_rois) / (1. + merge_weight2) * (actness > 0.).reshape((-1, 1))
